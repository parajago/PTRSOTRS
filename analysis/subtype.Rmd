...
ospredictedexpressionmashr <- read.table("Input/predixcan-predicted-output/pred_expr.BRCA-mashr.txt", header=TRUE, as.is = TRUE)
ospredictedexpressionmreference <- ospredictedexpressionmashr
ospredictedexpressionmashr$IID <- substr(ospredictedexpressionmashr$IID, 1, 12)
ospredictedexpressionmashr <- ospredictedexpressionmashr %>% dplyr::filter (IID %in% PTRSOSgroup)
...
ospredictedexpressionmashr <- as.data.frame(t(ospredictedexpressionmashr))
names(ospredictedexpressionmashr) <- ospredictedexpressionmashr["IID",]
...
ospredictedexpressionmashr <- ospredictedexpressionmashr[-1,] #Removing the FID row
ospredictedexpressionmashr <- ospredictedexpressionmashr[-1,] #Removing the IID row 
ospredictedexpressionmashrgene <- rownames(ospredictedexpressionmashr)
ospredictedexpressionmashr <- as.data.frame(lapply(ospredictedexpressionmashr, as.numeric))
rownames(ospredictedexpressionmashr) <- ospredictedexpressionmashrgene
ospredictedexpressionmashr$gene <- row.names(ospredictedexpressionmashr)

ospredictedexpression <- merge(ospredictedexpression, PTPRSweights, by="gene", all.x=TRUE)
ospredictedexpressionmashr <- merge(ospredictedexpressionmashr, PTPRSweights, by="gene", all.x=TRUE)
...
#Controls for Predicted-TPRS score - overall transcriptome and overall survival
...
PFS_European <- recurrencewhiteall %>% select(oss.months, phenotype, age, AJCCN, AJCCT, AJCCM, pam50luma, pam50her2, pam50basal)
colnames(PFS_European) = c("oss.months", "phenotype", "Age", "AJCCN.Nodal_Status", "AJCC.Tumor_Size", "AJCC.Metastasis", "LumA.PAM50","HER2.PAM50", "Basal.PAM50")
surv_objectall <- Surv(time=PFS_European$oss.months, event=as.numeric(PFS_European$phenotype))

fit.coxphall <- coxph(surv_objectall ~ PFS_European$Age+PFS_European$AJCCN.Nodal_Status+PFS_European$AJCC.Tumor_Size+PFS_European$LumA.PAM50+PFS_European$HER2.PAM50+PFS_European$Basal.PAM50, data=PFS_European) 
